...
banner() {
    ...
    if ! [ -x "$(command -v figlet)" ]; then
        echo 'Introducing HBomb'
    else
        figlet HBomb
    fi
    if ! [ -x "$(command -v toilet)" ]; then
        echo -e "\e[4;34m This Bomber Was Created By \e[1;32mHorrid Hanu \e[0m"
    else
        echo -e "\e[1;34mCreated By \e[1;34m"
        toilet -f mono12 -F border Hanu
    fi
    echo -e "\e[1;34m For Any Queries Join Me!!!\e[0m"
    echo -e "\e[1;32m           whatsapp: +919910384164 || Don't try to spam\e[0m"
    echo " "
